...
int main(int argc, char *argv[]) {
    ...
    for (;;) {
        ...
        if (millis() - prev_millis_1s_timer >= interval)    // one second update timer
        {
            ...
            Tx_fifo[0] = (uint8_t) 0xf0;   //split 32-Bit timestamp to 4 byte array
            Tx_fifo[1] = (uint8_t) 0x92;
            Tx_fifo[2] = (uint8_t) 0x49;
            Tx_fifo[3] = (uint8_t) 0x26;
            Tx_fifo[4] = (uint8_t) 0xda;
            Tx_fifo[5] = (uint8_t) 0x4d;
            Tx_fifo[6] = (uint8_t) 0xa6;
            Tx_fifo[7] = (uint8_t) 0x82;
            Tx_fifo[8] = (uint8_t) 0x00;

            Pktlen = 0x09;                              //set packet len to 0x13
